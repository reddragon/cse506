...
send_data(struct http_request *req, int fd)
{
	...
	memset(&url_stat, 0, sizeof(struct Stat));
	if((r = fstat(fd, &url_stat)) < 0)
	{
		return r;
	}
...
send_file(struct http_request *req)
{
	...
	if(url_stat.st_isdir)
	{
		r = send_error(req, 404);
		goto end;
	}
	file_size = url_stat.st_size;
